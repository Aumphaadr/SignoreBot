...
impl AuthManager {
    pub fn new(client_id: String, secrets: Secrets) -> Arc<Self> {
        ...
        Arc::new(Self { client_id: Mutex::new(client_id), secrets, slots: Mutex::new(slots), refresh_locks, tx })
    }
    ...
    pub fn status(&self, kind: AccountKind) -> AccountStatus {
        ...
        AccountStatus {
            ...
            missing_scopes: auth::missing_scopes(&scopes, Self::required_scopes(kind)),
            scopes,
    ...
    pub async fn validate_or_refresh(&self, kind: AccountKind) -> bool {
        ...
        match auth::validate(&pair.access_token).await {
            Ok(v) => {
                ...
                let missing = auth::missing_scopes(&v.scopes, Self::required_scopes(kind));
                if missing.is_empty() {
    ...
    pub async fn start_device_flow(self: &Arc<Self>, kind: AccountKind) -> Result<DeviceCode, AuthError> {
        let client_id = self.client_id();
        let scopes = Self::required_scopes(kind);
